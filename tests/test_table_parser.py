...
def test_table_parser_batch_classification(
    mock_load_model, mock_load_cls_model, mock_load_unet_model
):
    ...
    mock_load_model.side_effect = make_load_model_side_effect(
        mock_slanet, mock_unitable
    )

    mock_unet = MagicMock()
    mock_unet.predict.return_value = (
        "<table><tr><td>cell1-unet</td></tr></table>",
        np.array([[[0, 0], [10, 0], [10, 10], [0, 10]]], dtype=np.float32),
    )
    ...
    def batch_predict_side_effect(img_info_list, batch_size=16):
        ...
        img_info_list[1]["table_res"] = {
            "cls_label": "WirelessTable",
            "cls_score": 85.0,
        }
...
@patch("tdqeq.extractor.table_parser.TableParser._load_cls_model")
@patch("tdqeq.extractor.table_parser.TableParser._load_model")
def test_table_parser_batch_classification_fallback(
    mock_load_model, mock_load_cls_model, mock_load_unet_model
):
    ...
    mock_cls = MagicMock()
    mock_cls.batch_predict.side_effect = ValueError(
        "Input image smaller than target size"
    )
    mock_load_cls_model.return_value = mock_cls
...
@patch("tdqeq.extractor.table_parser.TableParser._load_cls_model")
@patch("tdqeq.extractor.table_parser.TableParser._load_model")
def test_table_parser_mode_tdqeq_double_plus(
    mock_load_model, mock_load_cls_model, mock_load_unet_model
):
    ...
    def unitable_side_effect(images, *args, **kwargs):
        res = MagicMock()
        res.pred_htmls = ["<table><tr><td>cell1-unitable</td></tr></table>"] * len(
            images
        )
        res.cell_bboxes = [
            np.array([[[0, 0], [10, 0], [10, 10], [0, 10]]], dtype=np.float32)
